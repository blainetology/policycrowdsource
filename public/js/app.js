...
var PCApp = __webpack_require__(35);
window.PCApp = PCApp;
...
function PCAppFunctions() {
			...
			$.get(url, null, function (data) {
				if (data['calculated']) {
					if (data['calculated'][2] == 'section') {
						...
						} else $('#ratingBoxSection' + data['calculated'][0]).removeClass('rating-calculated');
					} else if (data['calculated'][2] == 'document') {
						$('#ratingBoxDocument' + data['calculated'][0] + ' .rating-thumb').removeClass('calculated');
						if (data['calculated'][1]) $('#ratingBoxDocument' + data['calculated'][0] + ' .rating' + data['calculated'][1]).addClass('calculated');
					}
				}
			});
	...
	this.update_policy_section = function (policy_id, section_id, title, content) {
		if (!parent_section_id) parent_section_id = 0;
		axios.post('/update/policy/section', { _method: 'PUT', policy_id: policy_id, parent_section_id: parent_section_id }).then(function (response) {
			$('#subSections' + parent_section_id).append(response.data);
		}).catch(function (error) {
			console.log(error);
		});
	...
	this.update_rfp_section = function (rfp_id, section_id, title, content) {
		if (!parent_section_id) parent_section_id = 0;
		axios.post('/update/rfp/section', { _method: 'PUT', rfp_id: rfp_id, parent_section_id: parent_section_id }).then(function (response) {
			$('#subSections' + parent_section_id).append(response.data);
		}).catch(function (error) {
			console.log(error);
		});
	...
	this.update_question_section = function (question_id, section_id, content) {
		axios.post('/update/question/section', { _method: 'PUT', question_id: question_id, section_id: section_id, staged_content: content }).then(function (response) {}).catch(function (error) {
			console.log(error);
		});
	};
